...
figure, imagesc(image); axis image; colormap gray; hold on, title('patterns');
...
% image graph
figure, imagesc(abs(correlation)), axis image; colorbar, title('mean abs correlation matrix');
...
mask1 = correlation < threshold;
figure, imagesc(mask1); colorbar, title('mask_1');
...
figure, imagesc(mask2); colorbar, title('mask_2 [morphological operation]');
...
tempImage = image;
tempImage(mask2) = 255;
figure, imagesc(tempImage); colorbar, axis image; colormap gray, title('image before highlighting');
...
% 255 , in the red channel, so it will highlight the previus "all white" values
finalImage = cat(3, tempImage, image, image);
figure, imshowpair(image, finalImage, 'montage'), title('image and final defects');